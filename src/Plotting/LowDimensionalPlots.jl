...
# ------------------------------------------------------------------------------------------------ #
@userplot LowDim2
@recipe function f(h::LowDim2; method=principalcomponents, classlabels=false, features=getnames(h.args[1]))
    # show(size(F))
    F = h.args[1]
    # if h.args > 1
    #     Fs = h.args[2:end]
...
@userplot LowDim3
@recipe function f(h::LowDim3; method=principalcomponents, classlabels=true, features=getnames(h.args[1]))
    # show(size(F))
    F = h.args[1]
    fs = getnames(F)
    seriestype := scatter
